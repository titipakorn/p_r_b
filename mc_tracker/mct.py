    ...
    def __init__(self, num_sources, reid_model,
        ...
        for i in range(num_sources):
            self.scts.append(SingleCameraTracker(i, self._get_next_global_id,
                                                 ...
                                                 reid_model, visual_analyze=visual_analyze, **cfg.sct_config, **cfg.footfall_config))
            ...
            if dist < self.global_match_thresh:
                ...
                if all_tracks[idx1].id > all_tracks[idx2].id:
                    self.scts[all_tracks[idx1].cam_id].check_and_merge(
                        all_tracks[idx2], all_tracks[idx1])
                else:
                    self.scts[all_tracks[idx2].cam_id].check_and_merge(
                        all_tracks[idx1], all_tracks[idx2])
                assert i != j
                ...
                if track1.id != track2.id and track1.cam_id != track2.cam_id and \
                    ...
                    if not track1.f_orient.is_valid():
                        ...
                        f_complex_dist = track1.f_orient.dist_to_other(
                            track2.f_orient)
                    f_avg_dist = 0.5 * \
                        cosine(track1.f_avg.get(), track2.f_avg.get())
