...
def draw_detections(frame, detections, show_all_detections=True):
    ...
    for i, obj in enumerate(detections):
        ...
        box_color = COLOR_PALETTE[id %
                                  len(COLOR_PALETTE)] if id >= 0 else (0, 0, 0)
        ...
        if id >= 0:
            label = 'ID {}'.format(label) if not isinstance(
                label, str) else label
...
def visualize_multicam_detections(frames, all_objects, fps='', show_all_detections=True,
    ...
    label_size, base_line = cv.getTextSize(
        str(fps), cv.FONT_HERSHEY_SIMPLEX, 1, 2)
...
def plot_timeline(sct_id, last_frame_num, tracks, save_path='', name='', show_online=False):
    ...
    for track in tracks:
        ...
        if frame_ids[-1] > last_frame_num:
            frame_ids = [
                timestamp for timestamp in frame_ids if timestamp < last_frame_num]
        xy[track_id][frame_ids] = track_id
        xx = np.where(xy[track_id] == -1, np.nan, x)
        if track_id >= 0:
            color = COLOR_PALETTE[track_id % len(
                COLOR_PALETTE)] if track_id >= 0 else (0, 0, 0)
            color = [x / 255 for x in color]
        else:
            color = (0, 0, 0)
        colors.append(tuple(color[::-1]))
        plt.plot(xx, xy[track_id], marker=".",
                 color=colors[-1], label='ID#{}'.format(track_id))
    if save_path:
        file_name = os.path.join(
            save_path, 'timeline_{}.jpg'.format(plot_name))
