...
class OrientationFeature:
    def __init__(self, feature_len, initial_feature=(None, None)):
        ...
        self.orientation_features = [AverageEstimator()
...
class Track:
    ...
    def _interpolate(self, target_box, timestamp, skip_size):
        ...
        for t in range(1, skip_size):
            interp_box = [int(b1 + (b2 - b1) / skip_size * t)
    ...
    def merge_continuation(self, other, interpolate_time_thresh=0):
        ...
        if 1 < skip_size <= interpolate_time_thresh:
            self._interpolate(
                other.boxes[0], other.get_start_time(), skip_size)
...
class SingleCameraTracker:
        ...
        while len(tracks_indices) > 0:
            ...
            if dist < self.merge_thresh:
                kept_idx = self._concatenate_tracks(
                    tracks_indices[i], tracks_indices[j])
    ...
    def _get_updated_merge_distance_matrix_row(self, update_idx, ignore_idx, alive_indices):
        distance_matrix = THE_BIGGEST_DISTANCE * \
            np.ones(len(alive_indices), dtype=np.float32)
    ...
    def _concatenate_tracks(self, i, idx):
        if self.tracks[i].get_end_time() < self.tracks[idx].get_start_time():
            self.tracks[i].merge_continuation(
                self.tracks[idx], self.interpolate_time_thresh)
            ...
            self.tracks[idx].merge_continuation(
                self.tracks[i], self.interpolate_time_thresh)
            self.tracks[i] = None
            return idx
    ...
    def _compute_detections_assignment_cost(self, active_tracks_idx, detections, features):
        ...
        if self.analyzer and len(self.tracks) > 0:
            self.analyzer.prepare_distances(
                self.tracks, self.current_detections)
        ...
        for i, idx in enumerate(active_tracks_idx):
            ...
            for j, d in enumerate(detections):
                ...
                if self.tracks[idx].f_avg.is_valid() and features[j] is not None \
                    ...
                    reid_dist_avg = 0.5 * \
                        cosine(self.tracks[idx].f_avg.get(), features[j])
                    reid_dist_curr = 0.5 * \
                        cosine(
                    ...
                    if self.process_curr_features_number > 0:
                        ...
                        for s in range(start_index, num_features - 1, step):
                            if self.tracks[idx].features[s] is not None:
                                reid_dist_curr = min(
                                    reid_dist_curr, 0.5 * cosine(self.tracks[idx].features[s], features[j]))

                    reid_dist_clust = clusters_vec_distance(
                        self.tracks[idx].f_clust, features[j])
                    reid_dist = min(
                        reid_dist_avg, reid_dist_curr, reid_dist_clust)
                else:
                    reid_dist = 0.5
                cost_matrix[j, i] = iou_dist * reid_dist
                if self.analyzer:
                    self.analyzer.visualize_distances(
                        idx, j, [reid_dist_curr, reid_dist_avg, reid_dist_clust, 1 - iou_dist])
        if self.analyzer:
            self.analyzer.visualize_distances(
                affinity_matrix=1 - cost_matrix, active_tracks_idx=active_tracks_idx)
